...
class ViewController: UIViewController {
    ...
    private func detectObjects(in pixelBuffer: CVPixelBuffer) {
        ...
        // FILL IN HERE
        let request = VNCoreMLRequest(model: yoloModel) { [weak self] (vnRequest, error) in
            if let error = error {
                print("Failed to perform detection: \(error.localizedDescription)")
                return
            }
            self?.processDetectionResults(request: vnRequest)
        }
        // END TODO
    ...
    private func processDetectionResults(request: VNRequest) {
        ...
        // FILL IN HERE
        let filteredResults: [VNRecognizedObjectObservation]
        if let observations = request.results as? [VNRecognizedObjectObservation] {
            let confidenceThreshold: Float = 0.9
            filteredResults = observations.filter { observation in
            ...
            filteredResults = []
            print("Failed to get VNRecognizedObjectObservation from request or no results found.")
        }
        // END TODO
